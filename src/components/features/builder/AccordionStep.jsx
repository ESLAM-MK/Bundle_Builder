...
import { toggleStep } from '../../../store/slices/bundleSlice';
const AccordionStep = ({ stepNumber, nextTitle, title, isOpen, selectedCount,logo ,totalSteps ,products }) => {
            ...
            {isOpen && (
                <div className="p-[15px] md:pt-[15px] border-t border-gray-100 bg-[#EDF4FF]">
                    ...
                    <div className="flex flex-wrap justify-center gap-[15px] w-full">
                        ...
                        <div key={product.id} className="w-full md:w-[calc(20%-13px)]  lg:w-[calc(50%-7.5px)] ">
                    ...
                    <div className="w-full flex justify-center pt-6">
                      {nextTitle &&  <button
                            type="button"
                            className="cursor-pointer px-[24px] py-[5px] h-[39px] bg-transparent font-[400] font-['Gilroy-SemiBold'] border-[#4E2FD2] text-[#4E2FD2] border-[1px] rounded-[7px] leading-[24px] text-[18px]"
                        >
                            Next: {nextTitle}
                        </button>}
                    </div>
